...
| Publish workflow | **PASS** (inert until B-1/B-3) | `publish.yml` uses `Comfy-Org/publish-node-action@v1` + `secrets.REGISTRY_ACCESS_TOKEN`, triggers on pyproject change to main — correct per registry docs. Owner-guarded to `am_Vir` (see B-1). |
...
2. **Confirm/register registry publisher `@vir`**; plan to add `REGISTRY_ACCESS_TOKEN` secret on the GitHub repo (B-3).

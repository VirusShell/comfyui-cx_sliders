...
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        ...
        nodeType.prototype.onNodeCreated = function() {
            ...
            this.sliderY = 6; // Position near top since widget is hidden
            
            // Set initial node size (smaller without widget)
            this.size = [200, 40];
        ...
        nodeType.prototype.onConfigure = function(info) {
            if (info.properties) {
                ...
                this.sliderProps.step = Math.max(1, Math.round(info.properties.step ?? 1));
                this.sliderProps.snap = info.properties.snap ?? true;
                this.sliderProps.fillColor = isValidHexColor(info.properties.fillColor) ? info.properties.fillColor : "#4a90d9";
        ...
        nodeType.prototype._valueUpdate = function(e) {
            ...
            if (shouldSnap && this.sliderProps.step > 0) {
                const stepRatio = this.sliderProps.step / (this.sliderProps.max - this.sliderProps.min);
            ...
            // Calculate value
            let value = this.sliderProps.min + (this.sliderProps.max - this.sliderProps.min) * (this.unlock ? vX : clampedX);
            value = Math.round(value);
        ...
        nodeType.prototype.onDblClick = function(e, pos, canvas) {
            if (this._isInSliderBounds(e)) {
                canvas.prompt("Value", this.sliderProps.current, (v) => {
                    if (!isNaN(Number(v))) {
        ...
        // Compute size (prevent vertical resize from affecting layout)
        // Compute minimum size - return small values to allow free resizing
        nodeType.prototype.computeSize = function() {
            return [100, 40]; // Small min width allows shrinking
        };

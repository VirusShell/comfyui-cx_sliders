// ComfyUI - cxRangeSlider Float
// Two-handle range slider for floating-point values
...
app.registerExtension({
    name: "cxRangeSlider.Float",

    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name !== "cxRangeSliderFloat") return;
        ...
        nodeType.prototype.onNodeCreated = function() {
            ...
            this.properties.padding = this.rangeProps.padding;
        ...
        nodeType.prototype._getDecimals = function() {
            return getDecimalPlaces(this.rangeProps.padding);
        };
        ...
        nodeType.prototype.onPropertyChanged = function(name, value) {
            ...
            if (name === "low") {
                ...
                this.rangeProps.step = stepVal > 0 ? stepVal : 0.001;
                this.properties.step = this.rangeProps.step;
            } else if (name === "snap") {
        ...
        nodeType.prototype.onConfigure = function(info) {
            if (info.properties) {
                this.rangeProps.low = parseFloat(info.properties.low ?? 25.0);
                this.rangeProps.high = parseFloat(info.properties.high ?? 75.0);
                this.rangeProps.min = parseFloat(info.properties.min ?? 0.0);
                this.rangeProps.max = parseFloat(info.properties.max ?? 100.0);
                this.rangeProps.step = parseFloat(info.properties.step ?? 0.5);
                if (this.rangeProps.step <= 0) this.rangeProps.step = 0.001;
                this.rangeProps.snap = info.properties.snap ?? true;
                this.rangeProps.padding = info.properties.padding ?? "0.000";
                this.rangeProps.fillColor = isValidHexColor(info.properties.fillColor) ? info.properties.fillColor : "#d99a4a";
                this.rangeProps.borderColor = isValidHexColor(info.properties.borderColor) ? info.properties.borderColor : "#666666";
                this.rangeProps.textColor = isValidHexColor(info.properties.textColor) ? info.properties.textColor : "#ffffff";
        ...
        nodeType.prototype.onDrawForeground = function(ctx) {
            ...
            ctx.fillText(this._formatValue(low) + " - " + this._formatValue(high), width / 2, sliderY + sliderHeight / 2);
        };
        ...
        nodeType.prototype._getHandlePositions = function() {
            ...
            const lowRatio = range > 0 ? (this.rangeProps.low - this.rangeProps.min) / range : 0;
            const highRatio = range > 0 ? (this.rangeProps.high - this.rangeProps.min) / range : 1;
            return {
        ...
        nodeType.prototype._valueUpdate = function(e) {
            ...
            if (shouldSnap && this.rangeProps.step > 0) {
                const stepRatio = this.rangeProps.step / (this.rangeProps.max - this.rangeProps.min);
                vX = Math.round(vX / stepRatio) * stepRatio;
            }

            const clampedX = clamp(vX, 0, 1);
            let value = this.rangeProps.min + (this.rangeProps.max - this.rangeProps.min) * (this.unlock ? vX : clampedX);
            value = this._roundValue(value);
        ...
        nodeType.prototype.onDblClick = function(e, pos, canvas) {
            if (this._isInSliderBounds(e)) {
                ...
                const currentVal = handle === "low" ? this.rangeProps.low : this.rangeProps.high;

                canvas.prompt(handle === "low" ? "Low Value" : "High Value", this._formatValue(currentVal), (v) => {
                    if (!isNaN(Number(v))) {
        ...
        nodeType.prototype.onSerialize = function(info) {
            info.properties = {
                ...
                padding: this.rangeProps.padding,
        ...
        nodeType.prototype.getExtraMenuOptions = function(canvas, options) {
            ...
            options.push({
                ...
                callback: () => {
                    this.rangeProps.low = 25.0;
                    this.rangeProps.high = 75.0;
                    this.rangeProps.min = 0.0;
                    this.rangeProps.max = 100.0;
                    this.rangeProps.step = 0.5;
                    this.rangeProps.snap = true;
                    this.rangeProps.padding = "0.000";
                    this.rangeProps.fillColor = "#d99a4a";
                    this.rangeProps.borderColor = "#666666";
                    this.rangeProps.textColor = "#ffffff";

                    this.properties.low = 25.0;
                    this.properties.high = 75.0;

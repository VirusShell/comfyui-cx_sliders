    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        ...
        nodeType.prototype.onConfigure = function(info) {
            if (info.properties) {
                ...
                this.bankProps.step = Math.max(1, Math.round(info.properties.step ?? 1));
                ...
                this.bankProps.fillColor = isValidHexColor(info.properties.fillColor) ? info.properties.fillColor : "#4a90d9";
        ...
        nodeType.prototype.onDrawForeground = function(ctx) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                ctx.textAlign = "center";
                ctx.textBaseline = "middle";
                ctx.fillText(String(value), sb.barX + sb.barWidth / 2, sb.y + sb.height / 2);
            }
        };
        ...
        nodeType.prototype.onDblClick = function(e, pos, canvas) {
            ...
            for (let i = 0; i < this.bankProps.sliderCount; i++) {
                ...
                if (localX >= sb.barX && localX <= sb.barX + sb.barWidth &&
                    localY >= sb.y && localY <= sb.y + sb.height) {
                    const labels = this._getLabels();
                    canvas.prompt(labels[i] || ("Slider " + (i + 1)), this.bankProps.values[i], (v) => {
                        if (!isNaN(Number(v))) {

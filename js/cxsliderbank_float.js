// ComfyUI - cxSliderBank Float
// Vertically stacked slider bank for floating-point values with dynamic outputs
...
app.registerExtension({
    name: "cxSliderBank.Float",

    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name !== "cxSliderBankFloat") return;
        ...
        nodeType.prototype._syncPropsToProperties = function() {
            ...
            this.properties.padding = this.bankProps.padding;
        ...
        nodeType.prototype._getDecimals = function() {
            return getDecimalPlaces(this.bankProps.padding);
        };
        ...
        nodeType.prototype._roundValue = function(value) {
        ...
        nodeType.prototype._reconcileOutputs = function() {
            ...
            while (this.outputs.length < target) {
                this.addOutput("OUT_" + (this.outputs.length + 1), "FLOAT");
            }
        };
        ...
        nodeType.prototype._updateSize = function() {
            ...
            const contentHeight = this.buttonRowHeight + this.buttonGap +
                count * this.miniSliderHeight + Math.max(0, count - 1) * this.miniSliderGap;
            this.size = [220, contentHeight + 10];
        };
        ...
        nodeType.prototype._getButtonBounds = function() {
            ...
            return {
                ...
                remove: { x: startX + btnW + gap, y: y, width: btnW, height: this.buttonRowHeight }
            };
        };

        // Get mini-slider bounds for a given index (0-based)
        nodeType.prototype._getMiniSliderBounds = function(index) {
            const startY = this.buttonRowHeight + this.buttonGap;
            ...
            return {
                ...
                barWidth: this.size[0] - this.sidePadding * 2 - this.labelWidth - 4,
                y: y,
        ...
        nodeType.prototype.onPropertyChanged = function(name, value) {
            ...
            if (name === "sliderCount") {
                ...
                this.bankProps.min = parseFloat(value);
                this.properties.min = this.bankProps.min;
                for (let i = 0; i < MAX_SLIDERS; i++) {
                ...
                this.bankProps.max = parseFloat(value);
                this.properties.max = this.bankProps.max;
                for (let i = 0; i < MAX_SLIDERS; i++) {
                ...
                this.bankProps.step = stepVal > 0 ? stepVal : 0.001;
                this.properties.step = this.bankProps.step;
            } else if (name === "snap") {
                ...
                if (idx >= 0 && idx < MAX_SLIDERS) {
                    this._setSliderValue(idx, parseFloat(value));
                }
            }
        ...
        nodeType.prototype.onConfigure = function(info) {
            if (info.properties) {
                this.bankProps.sliderCount = clamp(info.properties.sliderCount ?? DEFAULT_COUNT, 1, MAX_SLIDERS);
                this.bankProps.min = parseFloat(info.properties.min ?? 0.0);
                this.bankProps.max = parseFloat(info.properties.max ?? 100.0);
                this.bankProps.step = parseFloat(info.properties.step ?? 0.5);
                if (this.bankProps.step <= 0) this.bankProps.step = 0.001;
                this.bankProps.snap = info.properties.snap ?? true;
                this.bankProps.padding = info.properties.padding ?? "0.000";
                this.bankProps.labels = info.properties.labels ?? "Slider 1,Slider 2,Slider 3,Slider 4,Slider 5,Slider 6,Slider 7,Slider 8";
                this.bankProps.fillColor = isValidHexColor(info.properties.fillColor) ? info.properties.fillColor : "#d99a4a";
                this.bankProps.borderColor = isValidHexColor(info.properties.borderColor) ? info.properties.borderColor : "#666666";
                this.bankProps.textColor = isValidHexColor(info.properties.textColor) ? info.properties.textColor : "#ffffff";

                for (let i = 0; i < MAX_SLIDERS; i++) {
                    const val = info.properties["value_" + (i + 1)];
                    this.bankProps.values[i] = val !== undefined ? parseFloat(val) : 0;
                }
        ...
        nodeType.prototype.onDrawForeground = function(ctx) {
            ...
            ctx.beginPath();
            ctx.roundRect(bounds.add.x, bounds.add.y, bounds.add.width, bounds.add.height, 3);
            ctx.fill();
            ...
            ctx.fillText("+", bounds.add.x + bounds.add.width / 2, bounds.add.y + bounds.add.height / 2);

            // Draw [-] button
            ctx.fillStyle = count > 1 ? "#6a4a4a" : "#3a3a3a";
            ctx.beginPath();
            ctx.roundRect(bounds.remove.x, bounds.remove.y, bounds.remove.width, bounds.remove.height, 3);
            ctx.fill();
            ...
            ctx.textAlign = "center";
            ctx.textBaseline = "middle";
            ctx.fillText("\u2212", bounds.remove.x + bounds.remove.width / 2, bounds.remove.y + bounds.remove.height / 2);
            ...
            for (let i = 0; i < count; i++) {
                ...
                ctx.textAlign = "center";
                ctx.textBaseline = "middle";
                ctx.fillText(this._formatValue(value), sb.barX + sb.barWidth / 2, sb.y + sb.height / 2);
            }
        };
        ...
        nodeType.prototype.onMouseDown = function(e, pos, canvas) {
            ...
            for (let i = 0; i < this.bankProps.sliderCount; i++) {
                const sb = this._getMiniSliderBounds(i);
                if (localX >= sb.barX && localX <= sb.barX + sb.barWidth &&
                    localY >= sb.y && localY <= sb.y + sb.height) {
        ...
        nodeType.prototype._sliderUpdate = function(index, localX) {
            ...
            let value = this.bankProps.min + (this.bankProps.max - this.bankProps.min) * vX;
        ...
        nodeType.prototype.onDblClick = function(e, pos, canvas) {
            ...
            for (let i = 0; i < this.bankProps.sliderCount; i++) {
                ...
                if (localX >= sb.barX && localX <= sb.barX + sb.barWidth &&
                    localY >= sb.y && localY <= sb.y + sb.height) {
                    const labels = this._getLabels();
                    canvas.prompt(labels[i] || ("Slider " + (i + 1)), this._formatValue(this.bankProps.values[i]), (v) => {
                        if (!isNaN(Number(v))) {
                            this._setSliderValue(i, parseFloat(v));
                            this.setDirtyCanvas(true, true);
                        }
                    }, e);
                    return true;
        ...
        nodeType.prototype.onSerialize = function(info) {
            info.properties = {
                ...
                padding: this.bankProps.padding,
        ...
        nodeType.prototype.computeSize = function() {
            ...
            const h = this.buttonRowHeight + this.buttonGap +
                count * this.miniSliderHeight + Math.max(0, count - 1) * this.miniSliderGap + 10;
            return [160, h];
        };
        ...
        nodeType.prototype.getExtraMenuOptions = function(canvas, options) {
            ...
            options.push({
                ...
                callback: () => {
                    ...
                    this.bankProps.min = 0.0;
                    this.bankProps.max = 100.0;
                    this.bankProps.step = 0.5;
                    this.bankProps.snap = true;
                    this.bankProps.padding = "0.000";
                    this.bankProps.labels = "Slider 1,Slider 2,Slider 3,Slider 4,Slider 5,Slider 6,Slider 7,Slider 8";

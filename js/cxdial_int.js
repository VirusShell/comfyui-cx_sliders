...
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        ...
        nodeType.prototype._getDialGeometry = function() {
            ...
            const pad = 10;
            const textSpace = 20;
            const radius = Math.min(width - pad * 2, height - pad * 2 - textSpace) / 2;
            const cx = width / 2;
            const cy = pad + radius;
            return { cx, cy, radius };
        };
        ...
        nodeType.prototype.onConfigure = function(info) {
            if (info.properties) {
                ...
                this.dialProps.step = Math.max(1, Math.round(info.properties.step ?? 1));
                this.dialProps.snap = info.properties.snap ?? true;
                this.dialProps.fillColor = isValidHexColor(info.properties.fillColor) ? info.properties.fillColor : "#4a90d9";
                this.dialProps.borderColor = isValidHexColor(info.properties.borderColor) ? info.properties.borderColor : "#666666";
                this.dialProps.textColor = isValidHexColor(info.properties.textColor) ? info.properties.textColor : "#ffffff";
        ...
        nodeType.prototype._mouseToValue = function(localX, localY) {
            ...
            return this.dialProps.min + (this.dialProps.max - this.dialProps.min) * ratio;
        };
        ...
        nodeType.prototype.onDblClick = function(e, pos, canvas) {
            ...
            if (this._isInDialBounds(localX, localY)) {
                canvas.prompt("Value", this.dialProps.current, (v) => {
                    if (!isNaN(Number(v))) {
